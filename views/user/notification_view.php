					...
					text:'Search:'
							...
							icon: '/images/icons/application_form_magnify.png',
 							cls:'x-btn-text-icon',
 					     	...
 					     	handler: notification.app.notificationEdit

 					 	}
 	    			 ]
 	    	});

 			notification.app.Grid = Grid;
 			notification.app.Grid.getStore().load({params:{start: 0, limit: 25}});
...
var _window = new Ext.Panel({
		...
		return {
 		...
 		notificationAdd: function(){
 			...
 			notification.app.notificationSetForm();
 		    ...
 		    _window = new Ext.Window({
 		        ...
 		        buttonAlign:'center',
 		        items: notification.app.Form,
 		        buttons: [{
 		         	text: 'Save',
                                icon: '/images/icons/disk.png',  cls:'x-btn-text-icon',

 	                handler: function () {
 			            if(ExtCommon.util.validateFormFields(notification.app.Form)){//check if all forms are filled up

 		                notification.app.Form.getForm().submit({
 			                success: function(f,action){
 				                ...
 				                ExtCommon.util.refreshGrid(notification.app.Grid.getId());
 				                _window.destroy();
 			                },
 		...
 		notificationEdit: function(){
 			...
 			if(ExtCommon.util.validateSelectionGrid(notification.app.Grid.getId())){//check if user has selected an item in the grid
 			var sm = notification.app.Grid.getSelectionModel();
 		    ...
 		    _window = new Ext.Window({
 		  	...
 		  	notification.app.Form.getForm().load({
		...
		return {
			...
			hiddenName: 'notification_type_id',
                        hiddenId: 'notification_type_id',
        ...
        setNoOfDays: function(){
                        ...
                        if(portion == 'FIRST HALF' || portion == 'SECOND HALF'){
                            notification.app.setNoOfDaysByPortion();
                            return;
                        }
			...
			if(objdate1 != "" || objdate2 != "")
			{
				...
				{
					dispvalue = (ddate1.getTime() > ddate2.getTime() ? "-"+(notification.app.round(totaldays)+1) : (notification.app.round(totaldays)+1));
 ...
 Ext.onReady(notification.app.init, notification.app);
