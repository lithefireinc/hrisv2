   ...
   // die($this->db->last_query());
    if($query->num_rows()>0){
      ...
      //  die($this->db->last_query());
        ...
        endforeach;
		//die(print_r($app_type));
        foreach($app_type as $key =>$value):
			//die(print_r($value));
        $fields = array("a.id as apps_pk","a.date_requested", "CONCAT(b.lastname, ', ', b.firstname) AS emp_name", "c.app_type", "c.id as audit_id");
